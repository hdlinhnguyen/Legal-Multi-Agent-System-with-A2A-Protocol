...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(
  ({ className, ...props }, ref) => (
    <textarea
      className={cn(
        "scrollbar-thin flex min-h-[44px] w-full resize-none rounded-md border-none bg-transparent px-3 py-2.5 text-base text-foreground placeholder:text-muted-foreground/70 focus-visible:outline-none focus-visible:ring-0 disabled:cursor-not-allowed disabled:opacity-50",
        className,
      )}
      ref={ref}
      rows={1}
...
>(({ className, sideOffset = 4, ...props }, ref) => (
  <TooltipPrimitive.Content
    ...
    className={cn(
      "fade-in-0 zoom-in-95 data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 z-50 animate-in overflow-hidden rounded-md border border-border bg-surface px-3 py-1.5 text-sm text-foreground shadow-md data-[state=closed]:animate-out",
      className,
    )}
    {...props}
  />
...
export const PromptInputBox = React.forwardRef(
  (props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
    ...
    const processFile = React.useCallback((file: File) => {
      const isDoc = /\.(pdf|docx?|md|txt)$/i.test(file.name) || file.type.includes("pdf") || file.type.includes("word") || file.type.includes("text");
      if (isDoc && onDocumentSelect) {
    ...
    const useExternalVoice = !!onVoiceToggle;
    const voiceActive = useExternalVoice ? isVoiceActive : isRecording;
    ...
    return (
      <>
        <PromptInput
          ...
          className={cn(
            "w-full rounded-3xl border border-border bg-surface shadow-soft p-3 transition-all duration-300 ease-in-out",
            voiceActive && "border-mint/60 shadow-glow",
            className,
          )}
          disabled={isLoading}
        ...
        >
          ...
          <div
            className={cn(
              ...
              voiceActive ? "opacity-90" : "opacity-100",
            )}
          >
            <PromptInputTextarea
              placeholder={
                voiceActive
                  ...
                  : showSearch
                    ? "Tra cứu tin tức & báo chí..."
                    : showThink
                      ? "Suy luận sâu với RAG..."
                      : showCanvas
          ...
          {voiceActive && useExternalVoice && (
            <div className="flex items-center gap-2 px-2 pb-1 text-xs text-mint">
          ...
          <PromptInputActions className="flex items-center justify-between gap-2 p-0 pt-2">
            <div
              className={cn(
                ...
                voiceActive ? "invisible h-0 opacity-0" : "visible opacity-100",
              )}
            >
              <PromptInputAction tooltip="Đính kèm tài liệu / ảnh">
                <button
                  ...
                  <div className="flex h-5 w-5 flex-shrink-0 items-center justify-center">
                    ...
                    >
                      <Globe
                        className={cn(
                          "h-4 w-4",
                          showSearch ? "text-mint" : "text-inherit",
                        )}
                      />
                    </motion.div>
                  </div>
                  <AnimatePresence>
                    {showSearch && (
                      <motion.span
                        ...
                        className="flex-shrink-0 overflow-hidden whitespace-nowrap text-mint text-xs"
                      >
                        Tin tức
                      </motion.span>
                    )}
                  ...
                  <div className="flex h-5 w-5 flex-shrink-0 items-center justify-center">
                    ...
                    >
                      <BrainCog
                        className={cn(
                          "h-4 w-4",
                          showThink ? "text-mint" : "text-inherit",
                        )}
                      />
                    </motion.div>
                  </div>
                  ...
                  <div className="flex h-5 w-5 flex-shrink-0 items-center justify-center">
                    ...
                    >
                      <FolderCode
                        className={cn(
                          "h-4 w-4",
                          showCanvas ? "text-mint" : "text-inherit",
                        )}
                      />
                    </motion.div>
                  </div>
              ...
              <Button
                ...
                className={cn(
                  ...
                  voiceActive
                    ...
                    : hasContent
                      ? "bg-mint text-white hover:bg-mint/90 shadow-glow"
                      : "bg-muted text-muted-foreground hover:bg-mint-soft hover:text-mint",
                )}
                onClick={() => {
                  if (voiceActive) {
                    if (useExternalVoice) onVoiceToggle?.();
                    else setIsRecording(false);
                  } else if (hasContent) handleSubmit();
                  else if (useExternalVoice && voiceSupported) onVoiceToggle?.();
                  else if (!useExternalVoice) setIsRecording(true);
                }}
                disabled={(isLoading && !hasContent) || (!voiceSupported && !hasContent && !voiceActive)}
              >
                {isLoading ? (
